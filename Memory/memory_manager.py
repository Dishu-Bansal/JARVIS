...
class FAISS:
    def __init__(self, index_file: str = "D:/SPECIAL/Jarvis/Memory/faiss/faiss_index.bin", metadata_file: str = "D:/SPECIAL/Jarvis/Memory/faiss/metadata.db"):
    ...
    def _load_index(self):
        ...
        if os.path.exists(self.index_file):
            ...
            self.last_modified = os.path.getmtime(self.index_file)
        else:
            self.index = faiss.IndexIDMap(faiss.IndexFlatIP(self.dimension))  # Inner Product (cosine similarity) index
    
    def _save_in_db(self, memory: str):
        try:
            conn = sqlite3.connect(self.metadata_file)
            cursor = conn.cursor()
            
            cursor.execute('''
                INSERT OR REPLACE INTO memories (memory)
                VALUES (?)
            ''', (memory,))
    ...
    def _get_memory_with_id(self, id):
    ...
    def save(self, query: str) -> bool:
        """Saves the given query into the Faiss Database

        Args:
            query: The query/memory to be saved

        Returns:
            A bool saying True is saved successfully or False if an error occurs
        """
        try:
            id = self._save_in_db(query)
            print(id)
            task = "retrieval.passage"
            query_embedding = self.model.encode([query], task=task, prompt_name=task, normalize_embeddings=True, device=torch.device("cuda"))
            self.index.add_with_ids(np.array(query_embedding, dtype='float32'), np.array([id], dtype='int64'))
    ...
    def get(self, query: str, top_k: int = 5) -> str:
        """Retrieves top_k matches from Faiss database based on cosine similarity

        Args:
            query: The query/memory to be searched for
            top_k (optional, Default = 5): Number of matches to be returned

        Returns:
            A string representing memores retrieved seperated into lines. Each line represents a memory
        """
        # try:
        ...
        doc_best = {}     # doc_id -> (faiss_score, chunk_idx)
        for score, idx in zip(distances[0], indices[0]):
            if idx < 0: continue
            doc_id = self._get_memory_with_id(int(idx))
            if doc_id not in doc_best or score > doc_best[doc_id[0]][0]:
                doc_best[doc_id[0]] = (float(score), int(idx), str(doc_id[1]))
        
        if not doc_best:
            return []
        # 4) Prepare reranker inputs: (query, best_chunk_text)
        rerank_inputs = []
        doc_ids       = []
        for doc_id, (faiss_score, chunk_idx, memory) in doc_best.items():
            rerank_inputs.append((query, memory))
            doc_ids.append(doc_id)
        
        # print(rerank_inputs)
        # 5) Rerank with CrossEncoder
        rerank_scores = self.reranker.predict(rerank_inputs)  # list of floats
        
        # 6) Combine FAISS + reranker scores (tune weights as you like)
        combined = []
        for doc_id, (faiss_score, _, memory), rr_score in zip(doc_ids, doc_best.values(), rerank_scores):
            final_score = 0.4 * rr_score + 0.6 * faiss_score
            combined.append((final_score, doc_id, memory))
        
        # 7) Sort and take top_k
        combined.sort(key=lambda x: x[0], reverse=True)
        top_docs = combined[:top_k]
        
        # 8) Fetch full document records from DB
        results = []
        for score, doc_id, memory in top_docs:
            record = list([memory])  # returns your (id, device_id, filename, ..., ocr)
            record.append(score)
            results.append(record)
        
        final = ""
        for i in range(1, min(len(results)+1, top_k+1)):
            final += f"{i}) {results[i-1][0]}\n"
        return final
        # except Exception as e:
        #     return str(e)

class SQLite:
    def __init__(self, metadata_file: str = "D:/SPECIAL/Jarvis/Memory/sqlite/memory.db"):
        self.metadata_file = metadata_file

    def getAllTables(self) -> str:
        """Returns a dictionary of table names and their column names."""
        conn = sqlite3.connect(self.metadata_file)
        cursor = conn.cursor()
    ...
    def createTable(self, table_name: str, columns: List[str]) -> bool:
        """Creates a FTS5 table in SQLite if it doesnt exist with the specified name

        Args:
            table_name: Name of the table to be created
            columns: A list of column names to be added to that table
        ...
        try:
            ...
            schema = ", ".join(columns)
            sql = f"CREATE VIRTUAL TABLE IF NOT EXISTS {table_name} USING fts5({schema}, tokenize = 'porter');"
            cursor.execute(sql)
    ...
    def saveData(self, table_name: str, data: Dict[str, str]) -> bool:
        """Saves data into the mentioned Table in SQLite database

        Args:
            table_name: The table name in which to add data
            data: a dictionary representing column names and their corresponding values to be inserted
    ...
    def getData(self, table_name: str, query: str, limit: int = 5) -> str:
        """Gets data from the specified table using semantic match of the query from SQLite Database

        Args:
            table_name: The table to be searched
            query: The text to be searched
            limit (optional, Default = 5): Number of matches to be returned

        Returns:
            A string reprsenting each match as a line
        """
        conn = sqlite3.connect(self.metadata_file)
        cursor = conn.cursor()
        ...
        final = ""
        for row in results:
            final += " | ".join(str(v) for v in row.values())
            final += "\n"
        return final

    def update_table_schema(self, table_name: str, new_column_name: str, new_column_type: str='TEXT') -> str:
        """
        ...
        - new_column_type: str - SQLite data type (default 'TEXT')
        """
        conn = sqlite3.connect(self.metadata_file)
        cursor = conn.cursor()
        ...
        if new_column_name not in existing_columns:
            ...
            conn.commit()
            conn.close()
            return f"Added column '{new_column_name}' of type {new_column_type} to '{table_name}'"
        else:
            return f"Column '{new_column_name}' already exists in '{table_name}'"
    
    def update_row(self, table_name: str, update_conditions: dict[str, str], new_values: dict[str,str]) -> str:
        """
        Updates one or more columns in a row that matches the given conditions.

        Parameters:
        - table_name: str - Name of the table
        - update_conditions: dict - Conditions to match row(s) (e.g., {"id": 5})
        - new_values: dict - New values to set (e.g., {"content": "Updated"})
        """
        conn = sqlite3.connect(self.metadata_file)
        cursor = conn.cursor()
        ...
        where_clause = " AND ".join([f"{k} = ?" for k in update_conditions])
        query = f"UPDATE {table_name} SET {set_clause} WHERE {where_clause}"

        values = list(new_values.values()) + list(update_conditions.values())
        cursor.execute(query, values)

        conn.commit()
        conn.close()

        return f"Updated {cursor.rowcount} row(s) in '{table_name}' where {update_conditions}"
